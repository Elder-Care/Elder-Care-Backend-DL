...
def pre_process_img(image):
    ...
    image = tf.image.random_crop(image, [default_height-np.random.randint(0, 4), default_width-np.random.randint(0, 4), 1])
...
def __parse_function_csv(serial_exmp_):
    features_ = tf.io.parse_single_example(serial_exmp_,
                                        features={"image/label": tf.io.FixedLenFeature([], tf.int64),
                                                  "image/height": tf.io.FixedLenFeature([], tf.int64),
                                                  "image/width": tf.io.FixedLenFeature([], tf.int64),
                                                  "image/raw": tf.io.FixedLenFeature([default_width*default_height*channel]
                                                                                  , tf.int64)})
...
def main(argv):
    with tf.compat.v1.Session() as sess:
        summary_writer = tf.compat.v1.summary.FileWriter(tensorboard_path, sess.graph)
        ...
        handle = tf.compat.v1.placeholder(tf.string, shape=[], name='handle')
        iterator = tf.compat.v1.data.Iterator.from_string_handle(handle, data_set_train.output_types, data_set_train.output_shapes)
