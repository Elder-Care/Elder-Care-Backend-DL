...
class CNN_Model():
    def __init__(self, num_tags_=7, lr_=0.001, channel_=1, hidden_dim_=1024, full_shape_=2304, optimizer_='Adam'):
        ...
        self.x_input = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, None, None, self.channel], name='x_input')
        self.y_target = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None], name='y_target')
...
#         self.logits = self.res_net_layer()
    ...
    def cnn_layer(self):
        with tf.compat.v1.variable_scope("conv1"):
            conv1_weight = tf.compat.v1.get_variable('conv1_weight', [self.conv_size[0], self.conv_size[0],
                                                            self.channel, self.conv_feature[0]],
                                           dtype=tf.float32, initializer=self.initializer)
            conv1_bias = tf.compat.v1.get_variable('conv1_bias', [self.conv_feature[0]], dtype=tf.float32,
        ...
        with tf.variable_scope("conv2"):
            conv2_weight = tf.compat.v1.get_variable('conv2_weight', [self.conv_size[1], self.conv_size[1],
            ...
            pool2 = tf.nn.max_pool2d(conv2_relu, ksize=[1, self.maxpool_size[1], self.maxpool_size[1], 1],
        ...
        with tf.variable_scope("conv3"):
            ...
            pool3 = tf.nn.max_pool2d(conv3_relu, ksize=[1, self.maxpool_size[2], self.maxpool_size[2], 1],
    ...
    def optimizer(self, loss_, method=''):
        if method == 'Momentum':
            ...
            train_step_ = tf.compat.v1.train.AdamOptimizer(self.lr).minimize(loss_, name='train_step')
            print('Using ', method)
        else:
